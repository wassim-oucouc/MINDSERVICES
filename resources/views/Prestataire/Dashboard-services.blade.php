    ...
    <div class="p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-2xl font-bold">{{$statistic['total']}}</h3>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-2xl font-bold">{{$statistic['totalactif']}}</h3>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 mb-1">Services Inactifs</p>
                        <h3 class="text-2xl font-bold">{{$statistic['totalinactif']}}</h3>
                    </div>
...
@endif
        ...
        <div class="flex flex-wrap items-center justify-between mb-6">
            <div class="flex items-center space-x-4 mb-4 md:mb-0">
                <a href="/professional/creation/service">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="table-row hover:bg-indigo-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="ml-3">
                                        <div class="text-sm font-medium text-gray-900">{{$value->Description}}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a target="_blank" href="/service/details/{{$value->id}}">
                                    ...
                                    <button onclick = "openmodaledit({{$value->id}},{{json_encode($value->titre)}},{{json_encode($value->Description)}},{{$value->Prix}},{{json_encode($value->CategorieNom)}},{{$value->duration}},{{json_encode($value->availability)}})" class="text-indigo-600 hover:text-indigo-900 mx-1">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                        </svg>
                                    </button>
                                <form action="/professional/delete/service/{{$value->id}}" method="POST" class="inline">
                                    @csrf 
                                    @method('DELETE')
                                    <button class="text-red-600 hover:text-red-900 mx-1">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                                        </svg>
                                    </button>
                                </form>
                            </td>
...
@endif
            ...
            <form id="serviceForm" action = "/professional/services" method="POST" enctype="multipart/form-data" autocomplete="off">
                @csrf
                @method('PUT')
                <!-- Nom de Service -->
    ...
    if(list_errors)
{
    ...
    // Additional JavaScript for handling form submission, error display, and image preview can be added here.
</script>
